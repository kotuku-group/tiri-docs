...
Opens the file at `Path`, requesting access permissions specified by `Mode`.  If `Mode` is not set, the file is opened in read-only mode.  Raises an exception if the file cannot be opened.  The returned handle closes itself when bound to a to-be-closed variable (<<sec-automatic-file-closure>>).
...
[[sec-automatic-file-closure]]
=== Automatic File Closure
indexterm:[file system, automatic closure] indexterm:[close variables, file handles] File handles define the `pass:[__]close` metamethod.  Binding a handle to a to-be-closed variable (<<sec-to-be-closed-variables>>) closes it when the declaring scope exits, with the same effect as calling `file.close()`:
...
Closure occurs on every scope exit path, including `return`, `break`, `continue`, and error unwinding.
