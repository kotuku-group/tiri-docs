...
Long-string literals (`pass:[[[...]]]`) do not interpret escape sequences and the pattern reads exactly as written:
...
The expression `r'pass:[...]'` is equivalent to `regex.new('pass:[...]')` with the escaping convenience of a long string.  Single-quoted, double-quoted, long-bracket, and levelled long-bracket delimiters are all supported:
...
Locates the first match in `Text` and returns its position as a half-open span: the byte position of the match start, and the byte position immediately after the match end.  The match length is therefore `stop - start`, and the pair can be passed directly to `string.sub()` to recover the matched text.  If no match is found, no values are returned.  The optional `Offset` specifies the byte position at which to begin searching (default `0`).
...
Returns an iterator for use in `for` loops.  Each iteration yields three values: the match start position, the exclusive match stop position, and a capture array (or `nil` if no groups are defined).  The start and stop positions form a half-open span, as they do for `findFirst()`.
...
[.api]`capture, pass:[...] = rx.extract(Text, [Offset], [Flags])`
